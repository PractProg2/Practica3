...
Status graph_deepFirst (Graph *pg,long ini_id,long end_id,char *nodestraversed){
    ...
    pila = stack_init(node_free, node_copy, node_print);
    ...
    while (stack_isEmpty(pila) == FALSE){
        ...
        for (i=num-1; i>=0; i--){
            ...
            if ( node_getLabel(aux) != BLACK){
                /* Actualizamos en el grafo su predecesor */
                node_setLabel(aux, BLACK);
...
int main(int argc, char *argv[]){
    ...
    graph_deepFirst(grafo, id_ini, id_end, salida);
